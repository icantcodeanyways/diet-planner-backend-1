    ...
    def post(self, user_id):
        try:
            ...
            for i in range(len(selected_meals)):
                ...
                # Rest of the code to fetch meal information remains the same...
            ...
            # Variables
            model.food_amounts = Var(food_vars, bounds=(0, None))

            # Objective function
            model.objective = Objective(
                expr=sum(model.food_amounts[food_vars[i]] * food_calories[i]
                         for i in range(len(food_vars))),
                sense=maximize
            )

            # Constraints
            model.protein_constraint = Constraint(
                expr=sum(model.food_amounts[food_vars[i]] * food_protein[i]
                         for i in range(len(food_vars))) <= (user["required_protien"] / 3)
            )
            model.fat_constraint = Constraint(
                expr=sum(model.food_amounts[food_vars[i]] * food_fat[i]
                         for i in range(len(food_vars))) <= (user["required_fat"] / 3)
            )
            model.carbs_constraint = Constraint(
                expr=sum(model.food_amounts[food_vars[i]] * food_carbs[i]
                         for i in range(len(food_vars))) <= (user["required_carbs"] / 3)
            )

            # Additional constraints for minimum and maximum quantities
            model.min_quantity_constraint = Constraint(
                expr=sum(model.food_amounts[food_vars[i]] for i in range(len(food_vars))) >= 20
            )
            model.max_quantity_constraint = Constraint(
                expr=sum(model.food_amounts[food_vars[i]] for i in range(len(food_vars))) <= 150
            )
            ...
            for food in food_vars:
                amount = model.food_amounts[food].value
                print(food, amount)

            # Rest of the code to populate the diet plan remains the same...
